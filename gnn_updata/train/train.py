...
def train(data,prev):
    data.x = data.x.float()
    prev.x = prev.x.float()
    data.x = torch.cat((data.x[:, 0:4], data.x[:, 5:]), dim=1)
    prev.x = torch.cat((prev.x[:, 0:4], data.x[:, 5:]), dim=1)#rad_fil10和15需要这个
    model.train()
    optimizer.zero_grad()
...
for epoch in range(51):
    ...
    if epoch in [0, 10, 20, 30, 40, 50]:
        torch.save(model.state_dict(), f"/root/autodl-tmp/temporal_gnn2/train/model/fil_rad/woDp_v16_5k_filrad25_limit20_r327_{epoch}.pth")